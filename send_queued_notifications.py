...
def process_queue():
    ...
    if len(queue) == 1:
        ...
        print(f"جاري إرسال إشعار مجمع لـ {len(queue)} وظائف")
        try:
            message = messaging.Message(
                data={
                    'type': 'job',
                    'title': f"تم نشر {len(queue)} وظائف جديدة",
                    'body': "افتح التطبيق الآن لتفقد أحدث الوظائف المضافة التي قد تناسبك!"
                },
                condition="'all_users' in topics || 'topic_all_users' in topics || 'topic_iraq' in topics",
                android=messaging.AndroidConfig(priority='high'),
                apns=messaging.APNSConfig(payload=messaging.APNSPayload(aps=messaging.Aps(content_available=True)))
            )
            response = messaging.send(message)
            print(f"تم إرسال الإشعار المجمع بنجاح: {response}")
        except Exception as e:
            print(f"فشل في إرسال الإشعار المجمع: {e}")
